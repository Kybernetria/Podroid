...
## Controller guest SSH

`controller-core::guest_ssh` and `phonectl guest` implement the direct controller-to-guest path. OpenSSH is invoked with strict pre-enrolled host-key verification, public-key-only authentication, no PTY, agent, X11, local command, multiplexing, or forwarding, and bounded process/output limits. The client streams a one-use Headscale key to the guest enrollment helper over stdin without placing it in process arguments. This guest credential and endpoint are separate from the restricted Android-host management SSH planned by ticket #16.
