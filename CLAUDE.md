...
Optional downloadable boot profiles are composed in `profiles/DownloadableProfileRuntime.kt`. BuildConfig supplies one immutable Ed25519 trust root and canonical HTTPS origins from the non-secret Gradle properties `podroidProfileSigningKeyId`, `podroidProfileEd25519X509PublicKeyBase64`, `podroidProfileTrustEpoch`, and `podroidProfileCanonicalOrigins`; debug may remain bundled-only, while release packaging rejects absent or invalid trust configuration. The closed signed runtime contract is v1 aarch64/direct-boot/overlay-ext4/ready-v1 with an explicit nonempty QEMU/AVF backend set; the bundled storage lineage is `podroid-alpine-overlay-v1`. Signed v2 UEFI/NoCloud profiles use an explicit non-confusable v2 envelope/domain, immutable typed CAS preparation, manager-fenced QEMU-only activation, fixed cloud-disk/UEFI-vars initialization with crash recovery, and an exact cloud readiness contract. Reproducible Debian release inputs live under `profiles/debian-cloud`; they still do not constitute physical boot evidence. The singleton repository lives under `filesDir/profile-store-v1`; an irreversible lineage marker prevents a revoked or previously active download from silently falling back onto bundled boot artifacts. Preparation and diagnostics are read/prepare-only, while confirmation, activation, and rollback are available only through the separate local `ProfileLifecycleOperations` boundary implemented by `DefaultVmManager`, sharing its lifecycle mutex, fixed-runtime preflight, and application asset-tree lease. The Settings workflow uses that local authority; these operations are intentionally not exposed by `PodroidService` Binder endpoints.
...
3. Busybox `/sbin/init` reads `/etc/inittab` and starts **OpenRC**. The exact `sysinit`, `boot`, `default`, and `shutdown` runlevel directories and symlink targets are reconstructed from `build-rootfs/runlevels.lock` at build time (chroot-into-aarch64 doesn't work on an x86_64 builder).
...
- **One-time legacy normalization.** `init-podroid` runs the immutable `podroid-overlay-normalize /mnt/persist` before the overlay is stacked. The helper uses descriptor-relative no-follow traversal, fails closed on any inspection/mutation/close error, rejects hostile `.podroid`/marker symlinks, and atomically publishes the regular `/mnt/persist/.podroid/normalized` marker only after complete success. Failure leaves the marker absent and drops to recovery so the preserved upper can be retried.
- **Imperative hooks.** `podroid-migrate` (OpenRC, runs `before podroid-bootstrap`) invokes the immutable runner, bounded index, scripts, and static no-follow helper only from `/mnt/lower`. It validates the complete strictly ordered `migrations/index`, executes indexed `<v>.sh` hooks for `applied < v <= system-version`, then atomically advances `applied-version` through a descriptor-relative helper that rejects symlinked parents. **To ship a fixup in a new release**: add the version to `build-rootfs/files/etc/podroid/migrations/index` and add an idempotent `<versionCode>.sh`. Pure file additions/changes need NO script—the overlay union surfaces them.
